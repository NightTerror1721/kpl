...
namespace kpl::utils
{
	...
	void DataReader::_load(std::istream& input)
	{
		...
		while (input)
		{
			...
			Size count = static_cast<Size>(input.gcount());
			if (buffer[count - 1] == '\r')
				count--;
		...
		if (!lines.empty())
		{
			...
			databuffer.extract(reinterpret_cast<std::byte**>(&_chars));
	...
	char DataReader::_move_index(Int64 delta, bool persist)
	{
		...
		if (delta > 0)
		{
			offset += static_cast<Offset>(delta);
		...
		{
			if (static_cast<Int64>(offset) <= -delta)
			{
				offset = 0;
				line = _lines;
			}
	...
	std::vector<char> DataReader::peek(Int64 from, Int64 to) const
	{
		...
		std::vector<char> chars(static_cast<Size>(to - from));
